...
pub enum AnalyticsFiltersError {
    ...
    Status403(models::FindInstanceWorkloads400Response),
    Status404(models::FindInstanceWorkloads400Response),
    Status500(models::FindInstanceWorkloads400Response),
...
pub async fn analytics_usage(
    ...
    let p_header_ibm_api_version = ibm_api_version;
    let p_query_instance = instance;
    let p_query_interval_start = interval_start;
    let p_query_interval_end = interval_end;
    let p_query_backend = backend;
    let p_query_user_id = user_id;
    let p_query_simulators = simulators;
    ...
    if let Some(ref param_value) = p_query_interval_start {
        req_builder = req_builder.query(&[("interval_start", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_interval_end {
        req_builder = req_builder.query(&[("interval_end", &param_value.to_string())]);
    }
    if let Some(ref param_value) = p_query_backend {
        req_builder = match "multi" {
            "multi" => req_builder.query(
                &param_value
                    .iter()
    ...
    if let Some(ref param_value) = p_query_simulators {
