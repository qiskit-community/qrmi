...
) -> Result<std::collections::HashMap<String, serde_json::Value>, Error<GetBackendPropertiesError>>
{
    ...
    let p_query_calibration_id = calibration_id;
    ...
    if let Some(ref param_value) = p_query_calibration_id {
        req_builder = req_builder.query(&[("calibration_id", &param_value.to_string())]);
    }
    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
