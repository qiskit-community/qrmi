...
pub struct Client {
    ...
    /// HTTP client to interact with Quantum System API service
    ...
    pub(crate) s3_config_for_qsapi: Option<aws_sdk_s3::Config>,
}
...
pub struct ClientBuilder {
    ...
    s3_config_for_qsapi: Option<aws_sdk_s3::Config>,
}

impl ClientBuilder {
    /// Construct a new [`ClientBuilder`] with the specified URL where
    /// Quantum System API service is running.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    ///
    /// ```rust
    /// use quantum_system_api::{ClientBuilder, AuthMethod};
    ///
    ...
    ///
    /// Depending on the network configuration, the IP address used to access S3 may differ between access from the API client and access from the DA API service. In such cases, `s3_endpoint_url_for_qsapi` argument should specify the URL used when accessing S3 from the DA API service.
    ///
    /// # Example
    ///
    /// ```rust
    ...
    pub fn with_s3bucket(
        ...
        s3_endpoint_url_for_qsapi: Option<impl Into<String>>,
    ) -> &mut Self {
        ...
        if let Some(endpoint_for_qsapi) = s3_endpoint_url_for_qsapi {
            let s3_config_for_qsapi = config_builder
                .clone()
                .endpoint_url(endpoint_for_qsapi.into())
                .build();
            self.s3_config_for_qsapi = Some(s3_config_for_qsapi);
        }
    ...
    pub fn build(&mut self) -> Result<Client> {
        ...
        Ok(Client {
            ...
            s3_config_for_qsapi: self.s3_config_for_qsapi.clone(),
        })
    }
}
