...
use quantum_system_api::utils::{s3::S3Client, uuid};
...
static QS_STORAGE_TYPE: &str = "s3_compatible";
static QS_TIMEOUT: u64 = 3600;

#[derive(Parser, Debug)]
#[command(version = "0.1.0")]
#[command(about = "Quantum System API client library - Example")]
struct Args {
    /// backend name
    #[arg(short, long)]
    backend_name: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let client = ClientBuilder::new("http://0.0.0.0:8290")
        .with_auth(AuthMethod::IbmCloudIam {
            ...
            service_crn: "crn:v1:local:qsa_sim".to_string(),
    ...
    let job = json!({
        ...
        "timeout_secs": QS_TIMEOUT,
        "storage": {
            "input": {
                "presigned_url": input_url,
                "type": QS_STORAGE_TYPE,
            },
            "results": {
