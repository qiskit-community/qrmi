...
fn from_config_builds_resource_from_map() {
    let qrmi =
        IBMQiskitRuntimeService::from_config(valid_config()).expect("from_config should succeed");
...
fn from_config_honors_optional_keys() {
    ...
    let qrmi = IBMQiskitRuntimeService::from_config(config).expect("from_config should succeed");
...
fn from_config_missing_service_crn() {
    ...
    let err = IBMQiskitRuntimeService::from_config(config)
        .map(|_| ())
        .unwrap_err();
