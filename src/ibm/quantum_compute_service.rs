...
impl IBMQuantumComputeService {
    ...
    /// the `<backend_name>_` prefix: `QRMI_IBM_QCS_ENDPOINT`,
    /// `QRMI_IBM_QCS_IAM_ENDPOINT`, `QRMI_IBM_QCS_IAM_APIKEY`,
    /// `QRMI_IBM_QCS_SERVICE_CRN`.
    ///
    /// # Optional keys
    ///
    /// `QRMI_IBM_QCS_SESSION_MODE` (default: `"dedicated"`),
    /// `QRMI_IBM_QCS_SESSION_MAX_TTL` (default: `28800`),
    /// `QRMI_IBM_QCS_TIMEOUT_SECONDS` or `QRMI_JOB_TIMEOUT_SECONDS`,
    /// `QRMI_IBM_QCS_SESSION_ID` or `QRMI_JOB_ACQUISITION_TOKEN`.
    ///
    /// Each key above also accepts its fully-lowercased form (e.g.
    /// `qrmi_ibm_qcs_endpoint`) as a fallback if the exact-case key isn't
    /// present in the map.
