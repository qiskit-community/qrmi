...
impl ResourceType {
    ...
    fn create_qrmi(
        ...
        match self {
            ResourceType::IBMDirectAccess { .. } => Ok(Box::new(IBMDirectAccess::new(qpu_name)?)),
            ResourceType::QiskitRuntimeService { .. } => {
                Ok(Box::new(IBMQiskitRuntimeService::new(qpu_name)?))
            }
            ResourceType::PasqalCloud { .. } => Ok(Box::new(PasqalCloud::new(qpu_name)?)),
        }
    }
}
