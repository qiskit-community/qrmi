...
--
-- (C) Copyright IBM, Alice and Bob 2026
--
...
if #arg ~= 2 then
    ...
    print("Usage: lua example.lua <backend name> <qir input file>\n")
    os.exit(1)
end

-- Create a resource handle (corresponds to the real qrmi_resource_new)
local resource, err = qrmi.new(arg[1], "alice-bob-felis")
...
local qir_input = payload_file:read("*a")
payload_file:close()

local nbShots = 50
local averageNbPhotons = 4

local input_json = string.format(
