...
use clap::Parser;
use dotenv::dotenv;
use qrmi::{alice_bob::AliceBobFelis, models::Payload, models::TaskStatus, QuantumResource};
use std::fs::File;
use std::io::prelude::*;
...
#[command(about = "QRMI for Alice Bob Felis - Example")]
struct Args {
    /// backend name
    #[arg(short, long)]
    backend: String,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut qrmi = AliceBobFelis::new(&args.backend)?;
    println!(
